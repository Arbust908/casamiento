<script setup lang='ts'>

</script>

<template>
  <p class="mb-6 text-xl">El mejor regalo que nos podes hacer es venir y pasarla bien con nosotros.</p>
  <p class="mb-6">Si nos queres hacer un regalo te dejamos algunas opciones</p>
  <div class="grid grid-cols-2 gap-6 items-center">
    <p class="font-bold">Cuenta en pesos</p>
    <button class="border rounded-full px-4 py-2 flex items-center justify-between gap-x-2 text-slate-100 hover:(text-slate-200 bg-slate-200/25 shadow-xl border-yellow-300)">
      <span>Cuenta pesos</span>
      <i class="i-ri-file-copy-2-line" />
    </button>
    <p class="font-bold">Cuenta en dolares</p>
    <button class="border rounded-full px-4 py-2 flex items-center justify-between gap-x-2 text-slate-100 hover:(text-slate-200 bg-slate-200/25 shadow-xl border-yellow-300)">
      <span>CBU cuenta afuera</span>
      <i class="i-ri-file-copy-2-line" />
    </button>
  </div>
</template>

<style scoped lang='scss'>

</style>
